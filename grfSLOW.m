...
for i = txtNames
    ...
    curStr = struct();
    if contains(fileName,'walkO') & contains(fileName,'Sgrf')
        ...
        if prevRow == currentRow
            GRF_SLOW{currentRow, counter}=output;
            counter = counter + 1;
        else
            if prevRow > 1
                [GRF_SLOW,subCell] = compareSizes(GRF_SLOW,subCell);
            end
            counter = 1;
            prevRow = prevRow + 1;
            GRF_SLOW{currentRow, counter}=output;
...
function [GRF_SLOW,subCell] = compareSizes(GRF_SLOW,subCell)
[rfin,cfin] = size(GRF_SLOW);
