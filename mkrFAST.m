...
for i = txtNames
    ...
    curStr = struct();
    if contains(fileName,'walkO') & contains(fileName,'Fmkr')
        ...
        if prevRow == currentRow
            MKR_FAST{currentRow, counter}=output;
            counter = counter + 1;
        else
            if prevRow > 1
                [MKR_FAST,subCell] = compareSizes(MKR_FAST,subCell);
            end
            counter = 1;
            prevRow = prevRow + 1;
            MKR_FAST{currentRow, counter}=output;
...
function [MKR_FAST,subCell] = compareSizes(MKR_FAST,subCell)
[rfin,cfin] = size(MKR_FAST);
