...
for i = txtNames
    ...
    curStr = struct();
    if contains(fileName,'walkO') & contains(fileName,'Cgrf')
        ...
        if prevRow == currentRow
            GRF_COMF{currentRow, counter}=output;
            counter = counter + 1;
        else
            if prevRow > 1
                [GRF_COMF,subCell] = compareSizes(GRF_COMF,subCell);
            end
            counter = 1;
            prevRow = prevRow + 1;
            GRF_COMF{currentRow, counter}=output;
...
function [GRF_COMF,subCell] = compareSizes(GRF_COMF,subCell)
[rfin,cfin] = size(GRF_COMF);
