...
for i = txtNames
    ...
    curStr = struct();
    if contains(fileName,'walkO') & contains(fileName,'Smkr')
        ...
        if prevRow == currentRow
            MKR_SLOW{currentRow, counter}=output;
            counter = counter + 1;
        else
            if prevRow > 1
                [MKR_SLOW,subCell] = compareSizes(MKR_SLOW,subCell);
            end
            counter = 1;
            prevRow = prevRow + 1;
            MKR_SLOW{currentRow, counter}=output;
...
function [MKR_SLOW,subCell] = compareSizes(MKR_SLOW,subCell)
[rfin,cfin] = size(MKR_SLOW);
